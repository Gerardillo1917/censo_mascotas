...
try {
    ...
    if ($tiene_vacuna && !empty($_POST['vacunas']['nombre'])) {
        $stmt_vacuna = $conn->prepare("INSERT INTO vacunas (
            id_mascota, nombre_vacuna, fecha_aplicacion
        ) VALUES (?, ?, ?)");

        foreach ($_POST['vacunas']['nombre'] as $index => $nombre_vacuna) {
            if (!empty($nombre_vacuna) && !empty($_POST['vacunas']['fecha'][$index])) {
                $fecha_vacuna = $_POST['vacunas']['fecha'][$index];
                $stmt_vacuna->bind_param("iss", $id_mascota, $nombre_vacuna, $fecha_vacuna);
                $stmt_vacuna->execute();
            }
        }
        $stmt_vacuna->close();
