<?php
// Usamos __DIR__ para asegurar que la ruta a config.php sea correcta desde /admin/
require_once __DIR__ . '/../config.php'; 
// Asumimos que conexion.php y funciones.php son requeridos por config.php o deben ser incluidos explícitamente si no.
// Si config.php no incluye conexion.php, descomenta la siguiente línea:
require_once BASE_PATH . '/database/conexion.php';
// Si config.php no incluye funciones.php, descomenta la siguiente línea:
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (function_exists('sanitizar_input')) { // Verificar si la función existe
        $username = sanitizar_input($username);
    } else {
        $username = trim(htmlspecialchars($username)); // Fallback básico
    }
        ...
        try {
            ...
            if ($result->num_rows === 1) {
                ...
                if ($usuario['activo'] == 1 && $usuario['rol'] === 'admin' && password_verify($password, $usuario['password_hash'])) {
                    ...
                    $_SESSION['panel_admin_rol'] = $usuario['rol']; // Será 'admin'
            ...
            // En un entorno de producción, registrar $e->getMessage() en un log.
             error_log("Error en login_panel.php: " . $e->getMessage());
        }
    }
}
// $conn->close(); // Cerrar conexión solo si ya no se va a usar en la página.
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <style>
        /* Estilos idénticos a tu login.php original para consistencia visual */
        :root {
            --color-primario: #8b0180; /* Morado oscuro */
            --color-secundario: #6a015f; /* Morado más oscuro */
            --color-blanco: #ffffff;
            --color-texto: #333333;
            --color-footer: #f5f5f5;
        }
        ...
        .login-logo-container {
            ...
            margin: 0 auto; max-width: 350px; /* Ajustado para el título */
        }
        .login-logo { height: 50px; margin-right: 15px; }
        .login-title { color: var(--color-primario); font-weight: 700; margin: 0; font-size: 1.3rem; } /* Ajustado */
            ...
            <div class="login-card-body">
                ...
                <form method="post" action="login_panel.php">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="username" name="username" required placeholder="Usuario administrador" value="<?= isset($_POST['username']) ? htmlspecialchars($_POST['username']) : '' ?>">
                    </div>
                    <div class="mb-3">
